...
const menuItems = [
        ...
        { id : 4, name: 'AWS Services', icon : <RoomServiceIcon />, link : 'aws' },
];

const Sidebar = ({sideBar}) => {
  return (
    <div className={`h-screen border-r border-gray-300 transition-all duration-200 ease-out ${sideBar ? 'w-20' : 'w-1/5' }`}>
                ...
                menuItems.map((item) => {
                    return (
                        <NavLink to={item.link} className={({ isActive }) => isActive ? "flex gap-4 justify-start items-center bg-blue-100 p-4 rounded-md": "flex gap-4 justify-start items-center p-4 rounded-md hover:bg-blue-50"} key={item.id}>
                            <span>{item.icon}</span>
                            <p className={`text-nowrap ${sideBar ? 'hidden' : ''}`}>{item.name}</p>
                        </NavLink>
                    )
                })
            }
